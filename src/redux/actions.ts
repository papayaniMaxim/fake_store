import { Product, userInfo } from "../interface/interfaces";
...
export const CARD_PRODUCT_ITEM_COUNT_WAS_CHANGED =
...
export function deleteFromCard(id: string) {
  return {
...
export function cardProductCoutnChangeAction(product: Product, count: number) {
  return {
    type: CARD_PRODUCT_ITEM_COUNT_WAS_CHANGED,
    order: { product, count },
  };
}

export function addUserInfo(userInfo: userInfo) {
  return {
    type: ADD_USER_INFO,
    userInfo,
  };
